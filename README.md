...
每個 GitHub Release 另附 **`dongwu-mod-build-source.zip`**（`build_all.ps1 -Deploy` 所需之 `_ignite_mod` + `DongwuOdyssey.BepInEx` 原始碼，不含 `bin`/`obj`），解壓到遊戲根目錄後可本地建置。

---

## Steam 遊戲更新後（必讀）

Steam 驗證或更新會把 `AnimOdyssey_Data/StreamingAssets/Assets/game_data.ab` 等還原為**原版**（檔案通常明顯小於 mod 合併後的數 MB）。**BepInEx 啟動不會自動執行 Python**；若插件 log 正常但數值／卡池／裝備 mod 失效，多半是資料檔未重新合併。

### 建議步驟（依序）

1. **關閉遊戲**。
2. **（建議）** Steam → 遊戲內容 → **驗證遊戲檔案完整性**，取得與目前 `resourceVersion` 一致的原版資料。
3. **刪除舊 mod 備份**（若存在，避免 `--restore` 把舊版蓋在新版遊戲上）：
   - `AnimOdyssey_Data\StreamingAssets\Assets\game_data.ab.ignite_mod.bak`
   - `AnimOdyssey_Data\StreamingAssets\Assets\spriteatlas.ab.ignite_mod.bak`（若有）
4. **重新合併並部署**（擇一，與你安裝的 mod 組合一致）：
   - **與 `BepInEx\plugins` 一致（推薦）** — 在遊戲根目錄下：
     ```powershell
     cd "C:\Program Files (x86)\Steam\steamapps\common\Dongwu Odyssey\DongwuOdyssey.BepInEx"
     .\build_all.ps1 -Deploy
     ```
     終端應出現 `apply data mods at deploy time -> ...`；合併後 `game_data.ab` 應明顯大於原版。
   - **僅 Python、依 `mods_enabled.json`**：
     ```powershell
     ...
     ```
     `apply_mods.py --restore` 會提示：若剛更新遊戲，舊備份可能不相容，必要時改以 Steam 驗證還原後再合併。
5. **啟動遊戲**，檢查 `BepInEx\LogOutput.log`；開發者可確認 mod 相關 id（例如 `1510099`、`7599010`）是否出現在資料中。

### 僅使用 Release 預編譯 zip 的玩家

不必安裝 Python。請下載與目前遊戲 **rv** 相符的 Release，或重新套用該組合的 **`dongwu-patched-data.zip`** 覆蓋 `AnimOdyssey_Data\StreamingAssets\`。**勿混用**不同 Release 的 `dongwu-bepinex-plugins.zip` 與 `dongwu-patched-data.zip`。

### 不建議

- 只靠重開遊戲期待資料自動修復（未執行 `-Deploy` 或 `apply_mods.py` 時不會重寫 `game_data.ab`）。
- 在遊戲更新後仍用更新前的 `.ignite_mod.bak` 做 `--restore` 而不先刪除備份或驗證遊戲。
...
| `equipment_1510099`（混合） | `BepInEx/plugins/equipment_1510099/` | `HarmonyLib/KaxinadeSpiralStrikePatches.cs` | 螺旋追斬：單體傷害技後再動一次，再動期間自身傷害 -50% |

純 Harmony 插件內 DLL：`DongwuOdyssey.Mod.<mod_id>.dll`、`DongwuOdyssey.ModCore.dll`、`DongwuOdyssey.HarmonyLib.dll`（無 `dongwu-data-mod.txt`）。  
`equipment_1510099` 另有 `dongwu-data-mod.txt` 與資料 DLL。
...
| `equipment_1510099` | 卡西娜德之劍 | 傳奇武器數值、圖示、文案、螺旋追斬（Harmony） | `mods/equipment_kaxinade_defs.py`、`mods/mod_texts.py`、`HarmonyLib/KaxinadeSpiralStrikePatches.cs` | `apply_equipment_kaxinade`、`apply_kaxinade_texts` | `DongwuOdyssey.Mod.equipment_1510099.dll` + `DongwuOdyssey.HarmonyLib.dll` |
...
**卡西娜德之劍自訂圖示：** 覆蓋 `_ignite_mod/img/kaxinade_sword.png`（建議約 195×195、RGBA），再套用資料 mod：

```powershell
python _ignite_mod\apply_mods.py --restore
python _ignite_mod\apply_mods.py --only equipment_1510099
```

圖示會寫入 `ItemIcon` 圖集的 `Red_weapon_004` 格。若已安裝 BepInEx 插件，可改為 `DongwuOdyssey.BepInEx\build_all.ps1 -Deploy`（會依 `dongwu-data-mod.txt` 一併套用）。
...
| 檔案 | 用途 |
|------|------|
| `BepInEx-Unity-IL2CPP-*.zip` | BepInEx 本體 |
| `dongwu-bepinex-plugins.zip` | 已編譯插件（依組合篩選） |
| `dongwu-patched-data.zip` | 已合併的 `game_data.ab`、語系等（**不需 Python**） |
| `dongwu-mod-build-source.zip` | 開發建置用原始碼（`build_all.ps1 -Deploy`） |
| `INSTALL.md` | 該組合安裝說明 |

**Steam 遊戲更新後**請見上文「Steam 遊戲更新後」；玩家通常只需重下／重套對應 rv 的 `dongwu-patched-data.zip`。
